...
The SVM is also useful for image classification. In this part, we are going to use the famous MNIST dataset to classify the images. The MNIST dataset is composed of 70,000 images of handwritten digits from 0 to 9. Each image is 28x28 pixels. We are going to use the SVM with the gaussian kernel to classify the images.

We will start by normalizing the data and then use the GridSearchCV function from scikit-learn to find the best hyperparameter.
